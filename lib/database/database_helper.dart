...
import '../models/workout_model.dart'; // ✅ Ajout de votre modèle
...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    return await openDatabase(
      path,
      version: 5, // ✅ Version 5 pour inclure la table workouts
      onCreate: _createDB,
  ...
  Future<void> _createDB(Database db, int version) async {
    ...
    // ✅ Table Workouts (VOTRE module ajouté)
    await db.execute('''
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      // ✅ Ajout de la table workouts pour la version 5
      await db.execute('''
  ...
  // ============================================================
  // 🔹 PARTIE WORKOUTS (VOTRE module - INTÉGRÉE)
  // ============================================================
  ...
  // Statistiques
  Future<double> getTotalCaloriesThisWeek() async {
    try {
      final db = await database;
  ...
  Future<Map<String, double>> getWorkoutDistribution() async {
    try {
      final db = await database;
      final result = await db.rawQuery('''
      ...
      for (var row in result) {
        distribution[row['type'] as String] = (row['total_duration'] as num).toDouble();
      }
      return distribution;
    } catch (e) {
